...
function sendEmail2FACode($email, $code) {
    try {
        ...
        if ($mail->send()) {
            file_put_contents(_DIR_.'/email_success.log', "[".date('Y-m-d H:i:s')."] Code sent to: $email\n", FILE_APPEND);
            return true;
        }
        return false;
        
    } catch (Exception $e) {
        file_put_contents(_DIR_.'/email_error.log', "[".date('Y-m-d H:i:s')."] Error: " . $e->getMessage() . "\n", FILE_APPEND);
        return false;
    }
}
